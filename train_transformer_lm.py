...
def iter_predict(Xs, Ms):
    ...
    for xmb, mmb in iter_data(Xs, Ms, n_batch=n_batch_train, truncate=False, verbose=True):
        n = len(xmb)
        if n == n_batch_train:
            preds.append(np.argmax(sess.run(eval_mgpu_logits, {X_train:xmb, M_train:mmb}), 1))
        else:
            preds.append(np.argmax(sess.run(eval_logits, {X:xmb, M:mmb}), 1))
...
def log():
    ...
    tr_acc = accuracy_score(tr_preds, np.reshape(trX[:n_valid, :-1, 0], [-1]))
    va_acc = accuracy_score(va_preds, np.reshape(vaX[:, :-1, 0], [-1]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--desc', type=str, default='transformer_lm') # dir args
