...
def clf_model(X, M, Y, clf_token, n_vocab, n_class=3, n_special=3, n_ctx=512, n_embd=768, embd_pdrop=0.1, n_layer=12, n_head=12, 
    attn_pdrop=0.1, resid_pdrop=0.1, clf_pdrop=0.1, train=False, reuse=False):
    with tf.variable_scope('clf_model', reuse=reuse):
        ...
        clf_h = dropout(clf_h, clf_pdrop, train)
        clf_logits = clf(clf_h, n_class, train=train)
        clf_logits = tf.reshape(clf_logits, [-1, n_class])
